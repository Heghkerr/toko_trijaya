...
@section('title', 'Proses Refund Transaksi')
...
<form action="{{ route('transactions.refund.store', $transaction->id) }}" method="POST">
    @csrf
    <div class="row">
        ...
        <div class="col-md-8">
            <div class="card shadow mb-4">
                ...
                <div class="card-body">
                    <p>Pilih jumlah barang yang akan dikembalikan dananya.</p>
                        ...
                        <table class="table">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-center">Sisa utk Refund</th>
                                    <th class="text-center" style="width: 120px;">Jml Refund</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach($transaction->details as $index => $detail)
                                    ...
                                    <tr>
                                        <td>
                                            ...
                                            {{-- Input hidden untuk ID detail transaksi --}}
                                            <input type="hidden"
                                        ...
                                        <td>
                                            {{-- Input jumlah yang akan di-refund --}}
                                            <input type="number"
            ...
            <div class="card shadow mb-4">
                ...
                <div class="card-body">
                    <dl class="row">
                        ...
                        <dd class="col-7">{{ $transaction->transaction_code }}</dd>

                        <dt class="col-5">Tanggal</dt>
                        <dd class="col-7">{{ $transaction->created_at->format('d M Y, H:i') }}</dd>

                        <dt class="col-5">Kasir</dt>
                        <dd class="col-7">{{ $transaction->user->name ?? 'N/A' }}</dd>

                        <dt class="col-5">Total Awal</dt>
                        <dd class="col-7">Rp {{ number_format($transaction->total_amount, 0, ',', '.') }}</dd>
                    ...
                    {{-- Alasan Refund --}}
                    <div class="mb-3">
                        <label for="reason" class="form-label">Alasan Refund (Opsional)</label>
                        <textarea class="form-control"
                                  id="reason"
                                  name="reason"
                                  rows="3"
                                  placeholder="Contoh: Barang rusak, salah ukuran, dll.">{{ old('reason') }}</textarea>
                    </div>
